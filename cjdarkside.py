...
def post_score_v2(df):
  df = df.withColumn("predict_qty", when(col("DayofMonth") == 1, col("predict_qty") + col("predict_qty")/2)
                                  .when(col("DayofMonth") == 2, col("predict_qty") + col("predict_qty")/2.5)
                                  .when(col("DayofMonth") == 3, col("predict_qty") + col("predict_qty")/2.5)
                                  .when(col("DayofMonth") == 4, col("predict_qty") + col("predict_qty")/2.5)
                                  .when(col("DayofMonth") == 5, col("predict_qty") + col("predict_qty")/2.5)
                                  .otherwise( col("predict_qty") ))
  return df
...
def to_Clear_BinaryModel(df, m1, m2):
    predictionsC  = m1.transform(df)
    predictionsR  = m2.transform(df)
...
def evalEachClass2_BinaryClass(df, lower, upper, l):
    raw = df
    df2 = df.filter( (df.TotalQtySale  >= lower) & (df.TotalQtySale  <= upper) & (df.prediction_type  == 'Lower') ).groupBy('prediction_type').count().toPandas()
    df = df.filter( (df.TotalQtySale  >= lower) & (df.TotalQtySale  <= upper) & (df.prediction_type  == 'Lower') ).groupBy('prediction_type').mean().toPandas()
    df['Range'] = str(lower)+'-'+str(upper)
    df = df[['Range','prediction_type', 'avg(avgPriceDis)', 'avg(avgPrice)', 'avg(supPrice)', 'avg(predict_qty)', 'avg(prediction_diff)']]
    df['count_of_each_part'] = df2['count'][0] if len(df2) > 0 else 'null'
    df_lower = df
